...
import type { Project, AlumniGroup, AlumniMember, Settings } from '@/types/api';
...
export async function apiFetch<T>(endpoint: string, options: FetchOptions = {}): Promise<T> {
  ...
  try {
    ...
    const parsedLimit =
      limitParam && /^[1-9]\d*$/.test(limitParam) ? Math.min(Number(limitParam), 100) : undefined;
    const isFeatured = params.get('featured') === 'true';

    // Featured filtering happens client-side, so fetch enough candidates first.
    listParams.set('per_page', isFeatured ? '100' : String(parsedLimit ?? 100));
