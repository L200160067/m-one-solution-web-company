...
export function mapWordPressPostToAppPost(wpPost: WordPressPost): Post {
  ...
  const mapped = {
    ...
    content: sanitizeCmsHtml(wpPost.content.rendered),
    category: category,
 ...
 */
export function mapWordPressProjectToAppProject(
  wpProject: WordPressProject,
): Project {
  ...
  const categoryName =
    categories && categories[0] ? categories[0].name : "Project";
  ...
  const cleanDescription = wpProject.excerpt.rendered
    .replace(/<[^>]*>/g, "")
    .trim();
 ...
 */
export function mapWordPressServiceToAppService(
  ...
  const cleanShortDesc = wpService.excerpt.rendered
    .replace(/<[^>]*>/g, "")
    .trim();
  ...
  const mapped = {
    ...
    full_description: sanitizeCmsHtml(wpService.content.rendered),
    features: stringToArray(wpService.acf?.features),
    benefits: stringToArray(wpService.acf?.benefits),
    keywords: wpService.acf?.keywords
      ? wpService.acf.keywords.split(",").map((s) => s.trim())
      : [],
    image_url: image_url,
    image_thumb: image_thumb,
...
export function mapWordPressTeamMemberToAppTeamMember(
  wpTeam: WordPressTeamMember,
): TeamMember {
  if (!wpTeam) return {} as TeamMember;
  const avatar_url =
    wpTeam._embedded?.["wp:featuredmedia"]?.[0]?.source_url || "";
  const mapped = {
    id: wpTeam.id,
...
export function mapWordPressTestimonialToAppTestimonial(
  wpTestimonial: WordPressTestimonial,
): Testimonial {
  if (!wpTestimonial) return {} as Testimonial;
  const avatar_url =
    wpTestimonial._embedded?.["wp:featuredmedia"]?.[0]?.source_url || "";
  const mapped = {
    id: wpTestimonial.id,
  ...
  if (!result.success) {
    console.warn(
      "[validation] Invalid testimonial payload",
      result.error.format(),
    );
...
export function mapWordPressPartnerToAppPartner(
  wpPartner: WordPressPartner,
): Partner {
...
export function mapWordPressAlumniToAppAlumni(
  wpAlumni: WordPressAlumni,
): AlumniMember {
...
export function mapWordPressSettingsToAppSettings(
  ...
  if (!result.success) {
    console.warn(
      "[validation] Invalid settings payload",
      result.error.format(),
    );
